...
//     if ((!pkt->isRequest() || !pkt->req->hasVaddr() || !pkt->req->hasContextId()) && !pkt->isWriteback()) { //BPC

//         //TODO: Change to hasVaddr?
//         // if(pkt->isWriteback() && (pkt->req->hasContextId() || pkt->req->hasVaddr())) {
//         //     DPRINTF(LUKAS, "1: isRequest: %d, hasVaddr: %d, hasContextId: %d, isResponse: %d, hasPC: %d\n", pkt->isRequest(), pkt->req->hasVaddr(), pkt->req->hasContextId(), pkt->isResponse(), pkt->req->hasPC());
//         // }
//         return;
//     }
...
//     int context_id = access_type == 2 ? 0 : pkt->req->contextId();
...
//     if ((!pkt->isRequest() || !pkt->req->hasVaddr() || !pkt->req->hasContextId()) && !pkt->isWriteback()) { //BPC
//         // if(pkt->isWriteback() && (pkt->req->hasContextId() || pkt->req->hasVaddr())) {
//         //     DPRINTF(LUKAS, "2: isRequest: %d, hasVaddr: %d, hasContextId: %d, isResponse: %d, hasPC: %d\n", pkt->isRequest(), pkt->req->hasVaddr(), pkt->req->hasContextId(), pkt->isResponse(), pkt->req->hasPC());
//         // }
//         return;
//     }
...
//     // if (predictor->bypass(pkt->req->getPC(), casted_replacement_data->etr, false, pkt->req->contextId(), casted_replacement_data->valid)) {
//     if(!pkt->isWriteback()) {
//         if(predictor->bypass(pkt->getAddr(), access_type, casted_replacement_data->abs_max_etr, false, pkt->req->contextId(), casted_replacement_data->valid)) {
//             // DPRINTF(MockingjayDebug, "Cache miss ---- Bypass cache: PC: 0x%.8x\n", pkt->req->getPC());
//             return;
//         }
//     }
