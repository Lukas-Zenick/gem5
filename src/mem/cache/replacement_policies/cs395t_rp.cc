...
CS395TRP::invalidate(const std::shared_ptr<ReplacementData>& replacement_data) {
    std::shared_ptr<CS395TReplData> casted_replacement_data = std::static_pointer_cast<CS395TReplData>(replacement_data);
    casted_replacement_data->valid = false;
    casted_replacement_data->etr = 0;
}

void
CS395TRP::touch(const std::shared_ptr<ReplacementData>& replacement_data, const PacketPtr pkt, const ReplacementCandidates& candidates) {
...
CS395TRP::getVictim(const ReplacementCandidates& candidates) const {
    ...
    int victim_etr = std::static_pointer_cast<CS395TReplData>(victim->replacementData)->etr;
...
CS395TRP::instantiateEntry() {
   return std::shared_ptr<ReplacementData>(new CS395TReplData(_num_etr_bits));
}

void CS395TRP::reset(const std::shared_ptr<ReplacementData>& replacement_data) const {
    panic("Cant train Mockingjay's predictor without access information.");
}

void CS395TRP::touch(const std::shared_ptr<ReplacementData>& replacement_data) const {
    panic("Cant train Mockingjay's predictor without access information.");
}

void CS395TRP::reset(const std::shared_ptr<ReplacementData>& replacement_data, const PacketPtr pkt) {
...
void CS395TRP::touch(const std::shared_ptr<ReplacementData>& replacement_data, const PacketPtr pkt) {
