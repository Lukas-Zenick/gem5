#ifndef __MEM_CACHE_REPLACEMENT_POLICIES_RPC_HH__
#define __MEM_CACHE_REPLACEMENT_POLICIES_RPC_HH__
...
namespace gem5 {
    ...
    {
        protected:
            struct RPCReplData : ReplacementData
            {
                int8_t etr;
                 ...
                 */
                RPCReplData(const int num_bits) : etr(0), abs_max_etr((1 << (num_bits - 1)) - 1), valid(false) {}
            };

        public:
            typedef RPCParams Params;
            RPC(const RPCParams &params);
            ~RPC() = default;

            /** History Sampler */
            std::unique_ptr<rpc::SampledCache> sampled_cache;

            /** Reuse Distance Predictor */
            std::unique_ptr<rpc::ReuseDistPredictor> predictor;
...
#endif // __MEM_CACHE_REPLACEMENT_POLICIES_RP_HH__
