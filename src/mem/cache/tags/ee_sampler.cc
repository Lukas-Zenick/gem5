...
// uint64_t get_pc_signature(bool hit, uint64_t full_addr, uint64_t access_type, int core, int num_cpu) {
//         uint64_t sig = hit ? 1 : 0;
//         sig = sig << 2;
...
//         sig = sig << 3;
//         sig = sig | ((full_addr >> 3) & 0b111);

//         if (num_cpu > 1) { //TODO: MAKE DYNAMIC
//             sig = sig << 2;
//             sig = sig | core;
//         }
...
// void ReuseDistPredictor::train(uint64_t last_PC, bool sampled_cache_hit, uint8_t curr_timestamp, uint8_t last_timestamp, bool evict, uint64_t type) {
...
// uint16_t ReuseDistPredictor::predict(uint64_t addr, uint64_t access_type, bool hit, int core_id, int etr_inf) {
//     // uint64_t signature = get_pc_signature(PC, hit, false, core_id, _num_cpus) % num_entries;
//     uint64_t signature = get_pc_signature(hit, addr, access_type, core_id, _num_cpus) % num_entries;
...
// bool ReuseDistPredictor::bypass(uint64_t addr, uint64_t access_type, uint8_t max_etr, bool hit, int core_id, bool cache_line_valid) {
//     if (cache_line_valid) {
//         uint64_t signature = get_pc_signature(hit, addr, access_type, core_id, _num_cpus) % num_entries;
...
// bool SampledCache:: sample(uint64_t addr, uint64_t access_type, uint8_t *curr_timestamp, int set, uint16_t *last_PC, uint8_t *last_timestamp, bool hit, bool *evict, bool *sampled_hit, int core_id, uint64_t inf_rd) {
...
//         uint16_t hashed_pc = get_pc_signature(hit, addr, access_type, core_id, _num_cpus); 
...
bool SampledCache:: sample(uint64_t addr, uint64_t PC, uint8_t *curr_timestamp, int set, uint16_t *last_PC, uint8_t *last_timestamp, bool hit, bool *evict, bool *sampled_hit, int core_id, uint64_t inf_rd, bool log) {
    int log2_num_cache_sets = (int) std::log2(_num_cache_sets);
    int log2_num_sets = _log2_num_sampled_sets - _log2_sampled_internal_sets;
    uint64_t num_sets_mask = (1 << log2_num_sets) - 1;
    uint64_t num_sampled_internal_sets_mask = (1 << _log2_sampled_internal_sets) - 1;

    bool isSampled = is_sampled_set(set, log2_num_cache_sets, log2_num_sets);

    if(log) {
        DPRINTF(LUKAS, "isSampled: %d\n", isSampled);
    }
