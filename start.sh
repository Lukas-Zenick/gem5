...
pythonScript="./gem5-configs-395t/fs_spec06gap_with_sampling.py"
...
./build/X86/gem5.opt --outdir=./results/"$output" "$pythonScript" --bpred="$bpred" --cores="$cores" --start_core_type="$start_core_type" --switch_core_type="$switch_core_type" \
...
--ff="$ff" --warmup="$warmup" --roi="$roi" --init_ff="$init_ff" --max_rois="$max_rois" --benchmark="$benchmark"
